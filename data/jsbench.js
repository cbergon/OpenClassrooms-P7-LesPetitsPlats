...
const recipes = [
  ...
  const prettyRecipe = {
    ...recipe,
    name: Utils.normalize(recipe.name),
    description: Utils.normalize(recipe.description),
    appliance: Utils.normalize(recipe.appliance),
    ingredients: recipe.ingredients.map((ingredient) => ({
      ...ingredient,
      ingredient: Utils.normalize(ingredient.ingredient),
    })),
    ustensils: recipe.ustensils.map(Utils.normalize),
  };
  return prettyRecipe;
});
...
console.time("func search");
for (i = 0; i < iterCount; i++) {
  applyFuncSearch(recipes, globalSearch);
}
console.timeEnd("func search");
